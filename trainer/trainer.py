...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(self.data_loader):
            ...
            for met in self.train_metrics:
                self.train_metrics_tracker.update(met.__name__, met(output, target))
    ...
    def _valid_epoch(self, epoch):
        ...
        batch_metric_fns = [m for m in self.valid_metrics if not hasattr(m, 'global_metric')]
        global_metric_fns = [m for m in self.valid_metrics if hasattr(m, 'global_metric')]
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data, target) in enumerate(self.valid_data_loader):
                ...
                self.valid_metrics_tracker.update('loss', loss.item())
                for met in batch_metric_fns:
                    self.valid_metrics_tracker.update(met.__name__, met(output, target))
                #self.writer.add_image('input', make_grid(data.cpu(), nrow=8, normalize=True))

            if need_all_outputs:
                output_all = torch.cat(output_all)
                target_all = torch.cat(target_all)
                for met in global_metric_fns:
                    self.valid_metrics_tracker.update(met.__name__, met(output_all, target_all))
